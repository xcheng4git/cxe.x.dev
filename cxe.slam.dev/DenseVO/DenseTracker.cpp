...
namespace vo
{
	...
	bool DenseTracker::match(vo::PointSelection& reference, RgbdImagePyramid& current, Eigen::Affine3d& transformation)
	{
		Result result;
		result.Transformation = transformation;
	...
	bool DenseTracker::match(vo::RgbdImagePyramid& reference, vo::RgbdImagePyramid& current, vo::DenseTracker::Result& result)
	{
	...
	bool DenseTracker::match(vo::PointSelection& reference, vo::RgbdImagePyramid& current, vo::DenseTracker::Result& result)
	{
		...
		vo::util::Revertable<Sophus::SE3d> initial(inc);
		vo::util::Revertable<Sophus::SE3d> estimate;
		...
		char name[256];
		sprintf_s(name, "%.2lf_error.avi", current.timestamp());
		...
		cv::VideoWriter vw(name, CV_FOURCC('P', 'I', 'M', '1'), 30, video_frame.size(), false);
		float rgb_max = 0.0;
		float depth_max = 0.0;

		//std::stringstream name1;
		//name1 << std::setiosflags(std::ios::fixed) << std::setprecision(2) << current.timestamp() << "_ref.png";
		sprintf_s(name, "%.2lf__ref.png", current.timestamp());
		cv::imwrite(name, current.level(0).rgb);

		//std::stringstream name2;
		//name2 << std::setiosflags(std::ios::fixed) << std::setprecision(2) << current.timestamp() << "_cur.png";
		sprintf_s(name, "%.2lf_cur.png", current.timestamp());
		cv::imwrite(name, reference.getRgbdImagePyramid().level(0).rgb);
		//*/
		...
		for (itctx_.Level = cfg.FirstLevel; itctx_.Level >= cfg.LastLevel; --itctx_.Level)
		{
			...
			do
			{
				...
				if (debug)
				{
					vo::computeResidualsAndValidFlagsSse(first_point, last_point, cur, K, transformf, wref, wcur, compute_residuals_result);
				}
				else
				{
					vo::computeResidualsSse(first_point, last_point, cur, K, transformf, wref, wcur, compute_residuals_result);
				}
				...
				//else {
				//	std::cerr << "Valid Constraints : " << n << std::endl;
				//}
				...
				{
					vo::computeWeightsSse(compute_residuals_result.first_residual, compute_residuals_result.last_residual, weights.begin(), mean, precision);
				}

				precision = vo::computeScaleSse(compute_residuals_result.first_residual, compute_residuals_result.last_residual, weights.begin(), mean).inverse();
	...
	cv::Mat DenseTracker::computeIntensityErrorImage(vo::RgbdImagePyramid& reference, vo::RgbdImagePyramid& current, const AffineTransformd& transformation, size_t level)
	{
		...
		vo::computeResidualsAndValidFlagsSse(first_point, last_point, cur, K, transformation.cast<float>(), wref, wcur, compute_residuals_result);
