...
namespace vo
{
	...
	static void pyrDownMeanSmoothIgnoreInvalid(const cv::Mat& in, cv::Mat& out)
	{
		...
		for (int y = 0; y < out.rows; ++y)
		{
			for (int x = 0; x < out.cols; ++x)
			{
				...
				//if(isfinite(in.at<T>(y0, x0)))
				if (_finite(static_cast<double>(in.at<T>(y0, x0))))
				{
					total += in.at<T>(y0, x0);
					cnt++;
				}

				//if(isfinite(in.at<T>(y0, x1)))
				if (_finite(static_cast<double>(in.at<T>(y0, x1))))
				{
					total += in.at<T>(y0, x1);
					cnt++;
				}

				//if(isfinite(in.at<T>(y1, x0)))
				if (_finite(static_cast<double>(in.at<T>(y1, x0))))
				{
					total += in.at<T>(y1, x0);
					cnt++;
				}

				//if(isfinite(in.at<T>(y1, x1)))
				if (_finite(static_cast<double>(in.at<T>(y1, x1))))
				{
					total += in.at<T>(y1, x1);
					cnt++;
				}
	...
	const vo::IntrinsicMatrix& RgbdCamera::intrinsics() const
	{
		return intrinsics_;
	}
	...
	RgbdCameraPyramid::RgbdCameraPyramid(size_t base_width, size_t base_height, const vo::IntrinsicMatrix& base_intrinsics)
	{
	...
	void RgbdCameraPyramid::build(size_t levels)
	{
		...
		for (size_t idx = start; idx < levels; ++idx)
		{
			...
			vo::IntrinsicMatrix intrinsics(previous->intrinsics());
	...
	*/
	...
	void RgbdImage::warpIntensity(const AffineTransform& transformationd, const PointCloud& reference_pointcloud, const IntrinsicMatrix& intrinsics, RgbdImage& result, PointCloud& transformed_pointcloud)
	{
		...
		for (size_t y = 0; y < height; ++y)
		{
			for (size_t x = 0; x < width; ++x, ++idx, ++warped_intensity_ptr, ++warped_depth_ptr)
			{
				...
				//if(!isfinite(p3d(2)))
				if (!_finite(static_cast<double>(p3d(2))))
				{
	...
	void RgbdImage::warpDepthForward(const AffineTransform& transformationx, const IntrinsicMatrix& intrinsics, RgbdImage& result, cv::Mat_<cv::Vec3d>& cloud)
	{
		...
		for (size_t y = 0; y < height; ++y)
		{
			...
			{
				//if(!isfinite(*depth_ptr))
				if (!_finite(static_cast<double>(*depth_ptr)))
				{
					continue;
				}
				...
				if (inImage(x_projected, y_projected))
				{
					...
					//if(!isfinite(warped_depth.at<DepthType>(yi, xi)) || (warped_depth.at<DepthType>(yi, xi) - 0.05) > depth)
					if (!_finite(static_cast<double>(warped_depth.at<DepthType>(yi, xi))) || (warped_depth.at<DepthType>(yi, xi) - 0.05) > depth)
	...
	void RgbdImage::warpDepthForwardAdvanced(const AffineTransform& transformation, const IntrinsicMatrix& intrinsics, RgbdImage& result)
	{
		...
		for (unsigned int idx = 0; idx < height * width; ++idx)
		{
			...
			if (!_finite(static_cast<double>(z))) continue;
			...
			for (int y = y_begin; y < y_end; ++y)
			{
				...
				{
					if (!_finite(static_cast<double>(*v)) || (*v) > z_transformed)
					{
