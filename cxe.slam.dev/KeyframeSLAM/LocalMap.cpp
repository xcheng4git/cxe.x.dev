...
namespace slam
{
	namespace internal
	{
		...
		struct LocalMapImpl
		{
			...
			vo::RgbdImagePyramid::Ptr keyframe_, current_;
			g2o::VertexSE3 *keyframe_vertex_, *previous_vertex_, *current_vertex_;

			g2o::SparseOptimizer graph_;
			int max_vertex_id_, max_edge_id_;

			slam::TrackingResultEvaluation::ConstPtr evaluation_;

			LocalMapImpl(const vo::RgbdImagePyramid::Ptr& keyframe, const AffineTransformd& keyframe_pose) :
				keyframe_(keyframe),
				keyframe_vertex_(0),
				previous_vertex_(0),
				current_vertex_(0),
			...
			g2o::VertexSE3* addFrameVertex(double timestamp)
			{
			...
			g2o::EdgeSE3* addTransformationEdge(g2o::VertexSE3 *from, g2o::VertexSE3 *to, const AffineTransformd& transform, const Matrix6d& information)
			{
				assert(from != 0 && to != 0);
	...
	LocalMap::Ptr LocalMap::create(const vo::RgbdImagePyramid::Ptr& keyframe, const AffineTransformd& keyframe_pose)
	{
		LocalMap::Ptr result(new LocalMap(keyframe, keyframe_pose));
		return result;
	}

	LocalMap::LocalMap(const vo::RgbdImagePyramid::Ptr& keyframe, const AffineTransformd& keyframe_pose) :
	...
	vo::RgbdImagePyramid::Ptr LocalMap::getKeyframe()
	{
		return impl_->keyframe_;
	}

	vo::RgbdImagePyramid::Ptr LocalMap::getCurrentFrame()
	{
		return impl_->current_;
	}

	void LocalMap::getCurrentFramePose(AffineTransformd& current_pose)
	{
		current_pose = getCurrentFramePose();
	}

	void LocalMap::setKeyframePose(const AffineTransformd& keyframe_pose)
	{
	...
	AffineTransformd LocalMap::getCurrentFramePose()
	{
	...
	void LocalMap::setEvaluation(slam::TrackingResultEvaluation::ConstPtr& evaluation)
	{
		impl_->evaluation_ = evaluation;
	}

	slam::TrackingResultEvaluation::ConstPtr LocalMap::getEvaluation()
	{
		return impl_->evaluation_;
	}

	void LocalMap::addFrame(const vo::RgbdImagePyramid::Ptr& frame)
	{
	...
	void LocalMap::addOdometryMeasurement(const AffineTransformd& pose, const Matrix6d& information)
	{
		impl_->addTransformationEdge(impl_->previous_vertex_, impl_->current_vertex_, pose, information);
	}

	void LocalMap::addKeyframeMeasurement(const AffineTransformd& pose, const Matrix6d& information)
	{
