...
#include "ConstraintProposalValidator.h"
...
namespace slam
{
	namespace internal
	{
		...
		public:
			...
			friend class ::slam::KeyframeGraph;
			...
			KeyframeGraphImpl() :
				...
				next_odometry_vertex_id_(INT_MAX),
				next_odometry_edge_id_(INT_MAX),
			...
			void configure(const slam::KeyframeGraphConfig& cfg)
			{
				cfg_ = cfg;
			...
			g2o::OptimizableGraph::Vertex* findNextVertex(g2o::OptimizableGraph::Vertex *v)
			{
				...
				double ts = dynamic_cast<Timestamped*>(v->userData())->timestamp;
			...
			void finalOptimization()
			{
				...
				{
					slam::constraints::ConstraintProposalVector constraints;
				...
				map_changed_(*me_);  // 这个信号我这没有链接到处理函数上

				{
					std::cerr << "created validation tracker instances: " << validator_pool_.size() << std::endl;
					//WCHAR szBuffer[1024];
					//swprintf(szBuffer, 1024, L"created validation tracker instances: %d \n", validator_pool_.size());
			...
			cv::Mat computeIntensityErrorImage(int edge_id, bool use_measurement)
			{
				...
				vo::DenseTracker tracker;
				tracker.configure(validation_tracker_cfg_);
				result = tracker.computeIntensityErrorImage(*kf2->image(), *kf1->image(), transform);

				std::map<int, slam::LocalTracker::TrackingResult>::iterator r = constraint_tracking_results_.find(edge_id);
			...
			void debugLoopClosureConstraint(int keyframe1, int keyframe2)
			{
				...
				slam::constraints::ConstraintProposalVector initial_proposals;
				initial_proposals.push_back(slam::constraints::ConstraintProposal::createWithIdentity(kf1, kf2));
				initial_proposals.push_back(slam::constraints::ConstraintProposal::createWithRelative(kf1, kf2));
				...
				slam::constraints::ConstraintProposalValidatorPtr& validator = validator_pool_.local();
		...
		private:
			...
			typedef tbb::enumerable_thread_specific<slam::constraints::ConstraintProposalValidatorPtr> ConstraintProposalValidatorPool;
			...
			void execOptimization()
			{
				...
				{
					slam::LocalMap::Ptr new_keyframe;
			...
			void newKeyframe(const LocalMap::Ptr& map)
			{
...

				{
					...
					std::cerr << "adding " << constraints.size() << " new constraints. " << std::endl;
					//WCHAR szBuffer[1024];
					//swprintf(szBuffer, 1024, L"adding  %d new constraints. \n", constraints.size());
					//OutputDebugString(szBuffer);
				}
				// update graph
				...
				//keyframegraph_.save("keyframegraph_2.g2o");
				...
				//keyframegraph_.save("keyframegraph_3.g2o");
				map_changed_(*me_);  //这个信号我这没有链接到处理函数上
			}

			slam::constraints::ConstraintProposalValidatorPtr createConstraintProposalValidator()
			{
				slam::constraints::ConstraintProposalValidatorPtr r = boost::make_shared<slam::constraints::ConstraintProposalValidator>();

				r->createStage(1)
					.trackingConfig(validation_tracker_cfg_)
					.keepAll()
					.addVoter(new slam::constraints::OdometryConstraintVoter())
					.addVoter(new slam::constraints::NaNResultVoter())
					.addVoter(new slam::constraints::ConstraintRatioVoter(cfg_.MinEquationSystemConstraintRatio))
					.addVoter(new slam::constraints::TrackingResultEvaluationVoter(cfg_.NewConstraintMinEntropyRatioCoarse))
					.addVoter(new slam::constraints::CrossValidationVoter(1.0))
					;
				...
				r->createStage(2)
					...
					.addVoter(new slam::constraints::NaNResultVoter())
					.addVoter(new slam::constraints::ConstraintRatioVoter(cfg_.MinEquationSystemConstraintRatio))
					.addVoter(new slam::constraints::TrackingResultEvaluationVoter(cfg_.NewConstraintMinEntropyRatioFine));
			...
			private:
				...
				slam::constraints::ConstraintProposalVector proposals_;

				slam::constraints::ConstraintProposalValidatorPtr& validator()
				{
					return validators_.local();
				}

				void appendProposals(slam::constraints::ConstraintProposalVector& pv)
				{
			...
			public:
				typedef tbb::blocked_range<slam::constraints::ConstraintProposalVector::const_iterator> ConstraintPoposalConstRange;
				...
				slam::constraints::ConstraintProposalVector& proposals()
				{
					return proposals_;
				}
			...
			void validateKeyframeConstraintsParallel(const KeyframeVector& constraint_candidates, const KeyframePtr& keyframe, slam::constraints::ConstraintProposalVector& proposals)
			{
				slam::constraints::ConstraintProposalVector initial_proposals;
				...
				{
					initial_proposals.push_back(slam::constraints::ConstraintProposal::createWithIdentity(keyframe, *it));
					initial_proposals.push_back(slam::constraints::ConstraintProposal::createWithRelative(keyframe, *it));
				}
			...
			int insertNewKeyframeConstraints(const slam::constraints::ConstraintProposalVector& proposals)
			{
				int inserted = 0;
				int max_distance = -1;

				for (slam::constraints::ConstraintProposalVector::const_iterator it = proposals.begin(); it != proposals.end(); ++it)
				{
					const slam::constraints::ConstraintProposalPtr& p = *it;
			...
			KeyframePtr insertNewKeyframe(const LocalMap::Ptr& m)
			{
				...
				kv->setUserData(new slam::Timestamped(keyframe->timestamp()));
			...
			{
				constraint_tracker_cfg_ = vo::DenseTracker::getDefaultConfig();
				...
				validation_tracker_cfg_ = vo::DenseTracker::getDefaultConfig();
			...
			vo::DenseTracker::Config validation_tracker_cfg_, constraint_tracker_cfg_;

			slam::KeyframeGraphConfig cfg_;
	...
	const slam::KeyframeGraphConfig& KeyframeGraph::configuration() const
	{
		return impl_->cfg_;
	}
	...
	void KeyframeGraph::configureValidationTracking(const vo::DenseTracker::Config& cfg)
	{
